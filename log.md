...
## ～2026/4/26
### 【やったこと】
- 日本戦スケジュール表の作成（match_schedule.html/php）
- 上記に対戦予想結果（AI予測）の画面を作成（action/ai_match_prediction_result.php）
  - OpenAIで秘密鍵作成
  - ローカルのみenv.phpを作成
- 診断結果画面の作成
- 「ワールドカップとは？」の画面を作成
- 日本代表の画面の一時的なページを作成
- AWSの設定（サーバーの用意）※本番環境への反映コマンドはノーションに書く
...
- メモ入力欄のXSS対策
  - エスケープ処理のため関数を記述
  - グローバルファンクションファイルを作成
  - confirm_country_detail.phpにその関数を記述
