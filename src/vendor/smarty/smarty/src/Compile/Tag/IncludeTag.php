...
class IncludeTag extends Base {
	...
	public function compile($args, \src\vendor\smarty\smarty\src\Compiler\Template $compiler, $parameter = [], $tag = null, $function = null): string
	{
		...
		if (preg_match('/^([\'"])(([A-Za-z0-9_\-]{2,})[:])?(([^$()]+)|(.+))\1$/', $source_resource, $match)) {
			...
			$handler = \src\vendor\smarty\smarty\src\Resource\BasePlugin::load($compiler->getSmarty(), $type);
		...
		if ($compiler->getTemplate()->caching && !$compiler->isNocacheActive()) {
			$_caching = \src\vendor\smarty\smarty\src\Template::CACHING_NOCACHE_CODE;
		}
