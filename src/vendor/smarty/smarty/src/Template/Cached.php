...
use src\vendor\smarty\smarty\src\Template\GeneratedPhpFile;
...
class Cached extends GeneratedPhpFile {
	 ...
	 *
	 * @param Source $source
	 * @param \src\vendor\smarty\smarty\src\Cacheresource\Base $handler
	 * @param $compile_id
	 * @param $cache_id
	 */
	public function __construct(Source $source, \src\vendor\smarty\smarty\src\Cacheresource\Base $handler, $compile_id, $cache_id) {
	 ...
	 *
	 * @param \src\vendor\smarty\smarty\src\Template $_template
	...
	public function isCached(Template $_template) {
		...
		while (true) {
			while (true) {
				...
				if ($this->valid && $_template->caching === \src\vendor\smarty\smarty\src\Smarty::CACHING_LIFETIME_CURRENT
					&& $_template->cache_lifetime >= 0 && time() > ($this->timestamp + $_template->cache_lifetime)
				) {
					// lifetime expired
					$this->valid = false;
				}
				if ($this->valid && $_template->compile_check === \src\vendor\smarty\smarty\src\Smarty::COMPILECHECK_ON
			...
			if ($this->valid && $_template->caching === \src\vendor\smarty\smarty\src\Smarty::CACHING_LIFETIME_SAVED
	...
	private function removeNoCacheHash(Template $_template, $no_output_filter) {
		...
		$codeframe = (new \src\vendor\smarty\smarty\src\Compiler\CodeFrame($_template))->create($content, '', true);
	...
	public function isFresh(Template $_template, array $properties): bool {
		...
		if (\src\vendor\smarty\smarty\src\Smarty::SMARTY_VERSION !== $properties['version']) {
			return false;
		}

		$is_valid = true;

		if (!empty($properties['file_dependency']) && ($_template->compile_check === \src\vendor\smarty\smarty\src\Smarty::COMPILECHECK_ON)) {
			$is_valid = $this->checkFileDependencies($properties['file_dependency'], $_template);
		}

		// CACHING_LIFETIME_SAVED cache expiry has to be validated here since otherwise we'd define the unifunc
		if ($_template->caching === \src\vendor\smarty\smarty\src\Smarty::CACHING_LIFETIME_SAVED && $properties['cache_lifetime'] >= 0
