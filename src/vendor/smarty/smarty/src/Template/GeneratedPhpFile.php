...
abstract class GeneratedPhpFile {
	 ...
	 * @throws \Exception
	 */
	protected function getRenderedTemplateCode(\src\vendor\smarty\smarty\src\Template $_template, $unifunc) {
		$level = ob_get_level();
		try {
			if (empty($unifunc) || !function_exists($unifunc)) {
				throw new \src\vendor\smarty\smarty\src\Exception("Invalid compiled template for '{$this->filepath}'");
			}
			$unifunc($_template);
		} catch (\Exception $e) {
	...
	protected function checkFileDependencies($file_dependency, Template $_template): bool {
		...
		foreach ($file_dependency as $_file_to_check) {
			...
			$handler = \src\vendor\smarty\smarty\src\Resource\BasePlugin::load($_template->getSmarty(), $_file_to_check[2]);
